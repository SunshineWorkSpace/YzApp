...
/**
 * 类名称:MallOrderBuyFragment
 * 类描述:
 * 创建时间: 2018-11-11-23:50
 * 创建人: sht
 * 修改人: sht
 * 修改备注:
 */
public class MallOrderBuyFragment extends BaseFragment implements TabLayout.OnTabSelectedListener,View.OnClickListener {
    private TabLayout tabLayout;
    private ViewPager viewPager;
    private String[] titles=new String[]{"所有订单","待付款","待发货","待收货","已完成","已取消"};
    ...
    private List<Fragment> getChildFragmentList() {
        ...
        for (int i = 0; i < titles.length; i++) {
            ...
            if("所有订单".equals(titles[i])){
                type="0";
            }else if("待付款".equals(titles[i])){
                type="1";
            }else if("待发货".equals(titles[i])){
                type="2";
            }else if("待收货".equals(titles[i])){
                type="3";
            }
            else if("已完成".equals(titles[i])){
                type="4";
            }
            else if("已取消".equals(titles[i])){
                type="5";
            }
            Bundle bundle = new Bundle();
            MallOrderBuyTabFragment featuredFragment = new MallOrderBuyTabFragment();
