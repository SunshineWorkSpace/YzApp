...
import com.yingshixiezuovip.yingshi.BuyPlayerProfileActivity;
...
import com.yingshixiezuovip.yingshi.SearchWLMessageActivity;
import com.yingshixiezuovip.yingshi.adapter.BuyerOrderAdapter;
import com.yingshixiezuovip.yingshi.adapter.SellerOrderAdapter;
...
/**
 * 类名称:MallOrderSellTabFragment
 * 类描述:
 * 创建时间: 2018-11-12-18:22
 * 创建人: sht
 * 修改人: sht
 * 修改备注:
 */
public class MallOrderSellTabFragment extends LazyFragment implements OnRefreshListener,OnLoadMoreListener, BuyerOrderAdapter.OnItemCleanClickListener, BuyerOrderAdapter.OnItemBuyClickListener {
...
//                layoutManager.invalidateSpanAssignments(); //防止第一行到顶部有空白区域
    ...
    private void loadData(){
        ...
        HttpUtils.doPost(TaskType.TASK_TYPE_MALL_SELL_LIST, params, this);
    }
    ...
    public void taskFinished(TaskType type, Object result, boolean isHistory) {
        ...
        switch (type){
            case TASK_TYPE_MALL_SELL_LIST:
                ...
                break;
            case TASK_TYPE_MALL_SELL_CLEAN:
                loadData();
                break;
    ...
    public void OnItemBuyClick(View view, int position) {
        if(mList.get(position).state.equals("1")){
            Intent it =new Intent(getActivity(),BuyPlayerProfileActivity.class);
            it.putExtra("id",mList.get(position).id);
            startActivity(it);
        } else if (mList.get(position).state.equals("2")) {
            Intent it = new Intent(getActivity(), SearchWLMessageActivity.class);
            it.putExtra("id", mList.get(position).id);
            startActivity(it);
            ...
            it.putExtra("id", mList.get(position).id);
            startActivity(it);
        }else if (mList.get(position).state.equals("4")) {
            Intent it = new Intent(getActivity(), LookLogisticsActivity.class);
            it.putExtra("id", mList.get(position).id);
            startActivity(it);
        }
    ...
    public void OnItemCleanClick(View view, int position) {
        if(mList.get(position).state.equals("1")){
            ...
            HttpUtils.doPost(TaskType.TASK_TYPE_MALL_SELL_CLEAN, params, this);
        }else if(mList.get(position).state.equals("2")||mList.get(position).state.equals("3")||mList.get(position).state.equals("4")||mList.get(position).state.equals("5")){
            Intent it =new Intent(getActivity(),BuyPlayerProfileActivity.class);
            it.putExtra("id",mList.get(position).id);
            startActivity(it);
        }
