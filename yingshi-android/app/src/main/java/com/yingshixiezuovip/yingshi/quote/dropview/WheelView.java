...
public class WheelView extends View {
    ...
    //    private static final float CENTERCONTENTOFFSET = 15;// 中间文字文字居中需要此偏移值
    private static final float CENTERCONTENTOFFSETY = 6;// 中间文字文字Y轴居中需要此偏移值
        private static final float CENTERCONTENTOFFSETX = 15;// 中间文字文字X轴居中需要此偏移值
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textColorOut = getResources().getColor(R.color.main_font_gray_color);
        textColorCenter = getResources().getColor(R.color.main_font_black_color);
        dividerColor = getResources().getColor(R.color.line_color);
    ...
    private void initPaints() {
        ...
        paintOuterCenterText = new Paint();
        paintOuterCenterText.setColor(textColorCenter);
        paintOuterCenterText.setAntiAlias(true);
        paintOuterCenterText.setTextScaleX(1.1F);
        paintOuterCenterText.setTypeface(Typeface.MONOSPACE);
        paintOuterCenterText.setTextSize(textSize);
    ...
    private void remeasure() {
        ...
        centerY = (measuredHeight + maxTextHeight) / 2.0F - CENTERCONTENTOFFSETY;
    ...
    private void measureTextWidthHeight() {
        Rect rect = new Rect();
        Paint paint = paintCenterText.getTextSize() > paintOuterText.getTextSize() ? paintCenterText : paintOuterText;
        for (int i = 0; i < adapter.getItemsCount(); i++) {
            ...
            paint.getTextBounds("\u661F\u671F", 0, 2, rect); // 星期
    ...
    public final void setTextSize(float size, float labelSize) {
        if (size > 0.0F) {
            textSize = (int) (context.getResources().getDisplayMetrics().density * size);
            labelTextSize = (int) (context.getResources().getDisplayMetrics().density * labelSize);
            paintOuterText.setTextSize(textSize);
            paintCenterText.setTextSize(labelTextSize);
            paintOuterCenterText.setTextSize(textSize);
        }
...
//        canvas.drawLine(0.0F, secondLineY, measuredWidth, secondLineY, paintIndicator);
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (label != null) {
                    float textWidth = paintOuterCenterText.measureText(contentText);
                    if (textWidth > 0)
                        canvas.drawText(label, measuredWidth / 2 + textWidth / 2 + CENTERCONTENTOFFSETX, centerY - 10, paintCenterText);
                }
                // 计算开始绘制的位置
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSETY, paintOuterCenterText);
                    canvas.restore();
    ...
    private void measuredCenterContentStart(String content) {
        Rect rect = new Rect();
        paintOuterCenterText.getTextBounds(content, 0, content.length(), rect);
        switch (mGravity) {
            case Gravity.CENTER:
